...
print(df.isnull().sum())  #finds the total noumber of null value in each column by adding number of true values(boolean values) given by isnull()
...
#there are 5 unique types of transactions. out of each type how many transactions are there?
print(df["type"].value_counts())
#there are users:customer and merchant in nameOrig and nameDest
#how many user are customer and how many are merchent in these column
print(df["nameOrig"].str.startswith("C", na=False).sum())
print(df["nameDest"].str.startswith("C", na=False).sum())

print(df["nameOrig"].str.startswith("M", na=False).sum())
print(df["nameDest"].str.startswith("M", na=False).sum())



#step3: no missing values in this dataset
#step 4:
...
raw_dashboard_dir = output_dir / "raw_dashboard"
...
if numeric_cols:
    ...
    fig.savefig(raw_dashboard_dir / "boxplot_dashboard.png", dpi=150, bbox_inches="tight")
    plt.show()
    ...
    plt.tight_layout(rect=[0, 0, 1, 0.97])
    fig.savefig(raw_dashboard_dir / "histogram_dashboard.png", dpi=150, bbox_inches="tight")
    plt.show()
    ...
    for ax, col in zip(axes, numeric_cols):
        sns.histplot(df[col], kde=True, ax=ax, color="green", bins=30)
        ax.set_title(col)
        ax.set_xlabel(col)
    ...
    plt.tight_layout(rect=[0, 0, 1, 0.97])
    fig.savefig(raw_dashboard_dir / "density_dashboard.png", dpi=150, bbox_inches="tight")
    plt.show()

if categorical_cols:
    # Dashboard 4: Bar plots for all categorical columns
    ...
    plt.tight_layout(rect=[0, 0, 1, 0.97])
    fig.savefig(raw_dashboard_dir / "categorical_bar_dashboard.png", dpi=150, bbox_inches="tight")
    plt.show()
